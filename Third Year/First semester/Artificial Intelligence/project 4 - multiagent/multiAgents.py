...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState: GameState, action):
        ...
        score = successorGameState.getScore() + foodReward + ghostPenalty
        ...
        def alphabeta(agentIndex, depth, state, alpha, beta):
            ...
            if agentIndex == 0:
                ...
                for action in state.getLegalActions(agentIndex):
                    ...
                    maxValue = max(maxValue, alphabeta(1, depth, successor, alpha, beta))
                    if maxValue > beta:
                        return maxValue
                ...
                for action in state.getLegalActions(agentIndex):
                    successor = state.generateSuccessor(agentIndex, action)
                    minValue = min(minValue, alphabeta(nextAgent, nextDepth, successor, alpha, beta))
                    if minValue < alpha:
                        return minValue
    ...
    def getAction(self, gameState: GameState):
        ...
        def expectimax(agentIndex, depth, state):
            ...
            if agentIndex == 0:
                ...
                for action in state.getLegalActions(agentIndex):
                    successor = state.generateSuccessor(agentIndex, action)
                    maxValue = max(maxValue, expectimax(1, depth, successor))
                return maxValue
                ...
                for action in legalActions:
                    successor = state.generateSuccessor(agentIndex, action)
                    totalValue += expectimax(nextAgent, nextDepth, successor)
...
def betterEvaluationFunction(currentGameState: GameState):
    ...
    if foodPositions:
        nearestFoodDistance = min(manhattanDistance(pacmanPosition, food) for food in foodPositions)
        foodScore = 10.0 / nearestFoodDistance
    else:
        foodScore = 0
    ...
    for idx, distance in enumerate(ghostDistances):
        if scaredTimers[idx] == 0:
            activeGhostPenalty += -200 / max(distance, 1)

    scaredGhostBonus = 0
    for idx, distance in enumerate(ghostDistances):
        if scaredTimers[idx] > 0:
            scaredGhostBonus += 200 / max(distance, 1)

    finalScore = baseScore + foodScore + activeGhostPenalty + scaredGhostBonus
